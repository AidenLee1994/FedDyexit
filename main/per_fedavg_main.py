...
from node.per_fedavg.server_node import Server_Node
from node.per_fedavg.client_node import Client_Node
...
def main():
    ...
    train_loader = DataLoader(DatasetSplit(dataset=train_dataset, idxs=user_groups[0], noise_switch=args.noise_switch,
                                           noise_level=args.noise_level, client_id=0,
                                           client_num=args.client_num_in_total), batch_size=args.batch_size)
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False)

    global_model = load_model(args.model_name, args.dataset_name)

    server = Server_Node(id=0, model=global_model, args=args, train_data_loader=train_loader,
    ...
    for i in range(1,args.client_num_in_total):
        train_loader = DataLoader(
            DatasetSplit(dataset=train_dataset, idxs=user_groups[i], noise_switch=args.noise_switch,
                         noise_level=args.noise_level, client_id=i, client_num=args.client_num_in_total),
    ...
    for round in range(args.comm_round):
        ...
        # selected client
        selected_indx = random_sampleing_clients(round_index=round, client_num_in_total=args.client_num_in_total,
        ...
        for idx in selected_indx:
            ...
            client_param=client.get_model_all_params()
        ...
        server.set_model_all_params(server_params)
        l.logging.info("update server success!")
        # update client params
        trans_update_start_time = time()
        for idx in selected_indx:
            client = client_list[idx]
            client.set_model_part_params(server_params)
