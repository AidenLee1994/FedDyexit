...
from node.fedbabu.server_node import Server_Node
from node.fedbabu.client_node import Client_Node
...
def main():
    ...
    for round in range(args.comm_round):
        ...
        # get server params
        server_param = server.get_model_part_params()
        ...
        for idx in selected_indx:
            ...
            client.set_model_part_params(copy.deepcopy(server_param))
            ...
            client_param = client.get_model_part_params()
        ...
        server.set_model_part_params(server_params)
        ...
        tmp_post_train_time = 0
        for idx in selected_indx:
            client = client_list[idx]

            train_start_time = time()
            # train client
            client.post_node_train()
            train_end_time = time()
            tmp_train_time = train_end_time - train_start_time
            tmp_post_train_time += tmp_train_time

        train_time += tmp_post_train_time / len(selected_indx)
        ...
        trans_sum +=server.cal_model_part_params_nums()
        l.logging.critical(
